...
int main() {
	...
	while (std::getline(file, line)) {
		...
		// We are doing integer division to find which multiple of 100 it is.
		// This does not account for zeroes.
		int newSum = sum + sign * val;
		count += std::abs(newSum) / 100 - sum / 100;
		// If we land on a zero, increment
		if (newSum == 0)
			count++;
		// If we CROSS zero while going from positive to negative, increment
		// once.
		if (sum != 0 && newSum < 0)
			count++;

		std::cout << "From: " << sum << ", To: " << (sum + sign * val)
